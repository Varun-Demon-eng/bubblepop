"""
=============================================================================
  TRAINING & EVALUATION SCRIPT FOR FREQUENCY FORENSICS CNN
  Dataset: HuggingFace "itsLeen/deepfake_vs_real_image_detection"
=============================================================================
"""
...
def main():
    print("=" * 65)
    print("  TRAINING FREQUENCY FORENSICS CNN ON 'itsLeen/deepfake_vs_real_image_detection'")
    print("=" * 65)
    ...
    print("[INFO] Loading dataset stream from HuggingFace...")
    ds = load_dataset("itsLeen/deepfake_vs_real_image_detection", streaming=True)
    train_stream = ds["train"]
    test_stream  = ds["test"]
    
    # Collect a balanced training batch
    MAX_TRAIN_SAMPLES = 400
    MAX_TEST_SAMPLES  = 100
    
    print(f"[INFO] Preparing {MAX_TRAIN_SAMPLES} training images...")
    X_train, y_train = [], []
    ...
    for item in train_stream:
        hf_label = item["label"]  # 0: Fake, 1: Real
        ...
        try:
            ...
            if len(X_train) % 50 == 0 or len(X_train) == MAX_TRAIN_SAMPLES:
                print(f"  Processed {len(X_train)}/{MAX_TRAIN_SAMPLES} train samples (Authentic: {real_count}, AI-Generated: {fake_count})")
    ...
    print(f"[INFO] Train set prepared: {len(X_train)} samples.")
    
    # Collect test batch
    print(f"[INFO] Preparing {MAX_TEST_SAMPLES} test images...")
    X_test, y_test = [], []
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"  Epoch {epoch}/{EPOCHS} [{elapsed:.1f}s] - Loss: {avg_loss:.4f} - Train Acc: {train_acc:.2f}%")
    ...
    print(f"[INFO] Model weights successfully saved to '{save_path.resolve()}'")
    
    # Re-evaluate user image
    user_img_path = r"C:\Users\raghu\OneDrive\Pictures\Saved Pictures\1000054739.jpg"
    print("\n" + "=" * 65)
    print(f"  RE-TESTING USER IMAGE: {user_img_path}")
    print("=" * 65)
